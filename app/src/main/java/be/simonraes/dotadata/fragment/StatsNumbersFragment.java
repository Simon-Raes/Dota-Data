...
public class StatsNumbersFragment extends Fragment implements View.OnClickListener, StatsMatchesLoader.ASyncResponseStatsLoader, DetailMatchLoader.DetailMatchLoaderDelegate {
    ...
    private void setGameModesGraph() {
        ...
        List<Map.Entry<String, Integer>> gameModesList = new ArrayList<Map.Entry<String, Integer>>(sortedMap.entrySet());

        for (int i = gameModesList.size() - 1; i >= 0; i--) {
            Map.Entry<String, Integer> entry = gameModesList.get(i);
            ...
            if (entry.getValue() > 0) {
                if (entry.getKey() != null) {
                    ...
                    if(gameModeName.equals("All Random Deathmatch")){
                        gameModeName = "AR Deathmatch";
                    }

                    txtLegend.setText(gameModeName + ": " + entry.getValue());
    ...
    private void showInfoDialog() {
        new AlertDialog.Builder(getActivity())
                ...
                .setMessage(Html.fromHtml("'Any gamemode' will show statistics based on all matches that had a first blood. <br>" +
                        "Special events and custom gamemodes will not be included."))
                .setCancelable(true)
